    ...
    if nd==2:
        ...
        for i in  range(para_to_scan["nSamp"][0]):
            for j in  range(para_to_scan["nSamp"][1]):
                tempinput = copy.deepcopy(input_original)
                tempinput["Sample_idx"] = samp_idx # to record the index of this sample in the whole parameter space.
                tempinput = generate_input(tempinput,para_to_scan,i,samp_idx)
                samp_idx+=1
...
def generate_input(tempinput,para_to_scan,i,j):
...
# 
input_fn = "/home/tianmu/Code/APES_dev/run_scripts/Run_tool/run_and_view_20210423/debug/input.txt"
tempinput = {}
tempinput = get_input(tempinput,input_fn)

working_dir = "/home/tianmu/Code/APES_dev/run_scripts/Run_tool/run_and_view_20210423/debug/"
...
working_folder = 'CEPC/TDR_Z_counter_3rf_45F6D9D_scan'
input_space = []
input_space = get_scan_para_space(para_to_scan,tempinput,input_space,working_dir)
...
nTrain = 1
gap = 0.0 # percentage of gap
n_bunches, pattern = get_beam_pattern(nTrain, n_bunches, gap,h[0],fill_step)
NperBunch = IbDC/n_bunches/f0/1.6e-19

RoQ = np.array([213/4,213/4+20,213/4])*nCav
delay = np.array([0,0,0])
n_fb_on = np.array([50.0,50,50])
gII = np.array([0.0,0,0])
gQQ = np.array([0.0,0,0])
gIQ = np.array([0.0,0,0])
gQI = np.array([0.0,0,0])
    ...
    for thetaL_factor in range(N_thetaL):
        ...
        # Now calculate the inputs
            ...
            if nRF==3:
                thetaL[0] = (ThetaL_min[0]+dThetaL[0]*thetaL_factor)/180.0*pi  # angle between Ig and Vc
                thetaL[1] = (ThetaL_min[1]+dThetaL[1]*thetaL_factor2)/180.0*pi  # angle between Ig and Vc
                thetaL[2] = (ThetaL_min[2]+dThetaL[2]*thetaL_factor2)/180.0*pi  # angle between Ig and Vc            
            Vbr = 2*IbDC*Rsh
