...
IbDC = -qPb*nBunch/T0
Ig = Vc/Z-IbDC*2
#Ig = Ig*np.exp(1j*cav1.wrf*Trf/360*0.5)
cav1.Igm1 = Ig
cav1.Vgm1 = Ig*Z
...
if debug2:
    ...
    for i in range(nRamp):
        ...
        cav1.update_Vg(t)
        cav2.update_Vg(t)
        cav1.kick_par_beamC(beam1, dynamic_on)
        #cav1.kick_par_beam(beam1, dynamic_on)
        OneTurn.rad_map_par(vRad, Gamma0, beam1.gammas, rad_on)
        OneTurn.oneTurnMap_par(GMTSQ, Gamma0, beam1.gammas, beam1.ts, T0)
        
        #for j in range(nBunch):
        #    cav1.kick_par(bunches[j],dynamic_on)
        #    OneTurn.rad_map(vRad,Gamma0, bunches[j],rad_on)
        #    OneTurn.oneTurnMap(GMTSQ, Gamma0, bunches[j], T0/nStation)
        #    if nStation == 2:
