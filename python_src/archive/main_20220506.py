...
h = np.array(inputs[inputs["Name"]=="h"]["Values"].iloc[0].split()).astype("int")
RoQ = np.array(inputs[inputs["Name"]=="RoQ"]["Values"].iloc[0].split()).astype("float")
QL = np.array(inputs[inputs["Name"]=="QL"]["Values"].iloc[0].split()).astype("float")
R = RoQ*QL
vRad = np.array(inputs[inputs["Name"]=="vRad"]["Values"].iloc[0].split()).astype("float")
vQuad =  np.array(inputs[inputs["Name"]=="vQuad"]["Values"].iloc[0].split()).astype("float")
vSync = vRad
df =  np.array(inputs[inputs["Name"]=="df"]["Values"].iloc[0].split()).astype("float")
...
for i in range(nStation):
    cavs.append(RF.Cav(fc[i],frf[i],RoQ[i],QL[i],0))
...
if debug2:
    ...
    for i in range(nRamp):
        ...
        for icav in range(nStation):
            ...
            OneTurn.rad_map_par(vRad[0], Gamma0, beam1.gammas, rad_on)
    ...
    dtheta = 2*k*IbDC*pattern[1]*fillStep*Trf_main/np.abs(Vc)/np.sin(np.angle(Vc))/np.pi*180
